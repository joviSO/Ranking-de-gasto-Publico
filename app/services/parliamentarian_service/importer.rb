class ParliamentarianService::Importer < ApplicationService
  ...
  def call(file)
    ...
    csv.each do |row|
      ...
      parliamentarians_id = Parliamentarian.find_by(ideCadastro: row['ideCadastro']).id
      vlrLiquido = row["vlrLiquido"]
      txtFornecedor = row["txtFornecedor"] 
      urlDocumento = row["urlDocumento"] 
      datEmissao = row["datEmissao"]
      
      BudgetService::Creater.new.call(parliamentarians_id, vlrLiquido, txtFornecedor, urlDocumento, datEmissao)
